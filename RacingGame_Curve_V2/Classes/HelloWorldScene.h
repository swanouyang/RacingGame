...
private:
	...
	double centrifugal;                     // centrifugal force multiplier when going around curves//¿Î–ƒ¡¶
...
public:
	...
	void resetVar();
	void addSegment(double curve);
	void addRoad(double enter, double hold, double leave, double curve);
	void addStraight(double num = ROAD_LENGTH_MEDIUM);
	void addCurve(double num = ROAD_LENGTH_MEDIUM, double curve = ROAD_CURVE_MEDIUM);
	void addSeqCurves();
	void resetRoad();
	void initVar();
	void initColor();
