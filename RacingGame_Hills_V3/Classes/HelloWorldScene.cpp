...
void HelloWorld::resetRoad() {
	...
	addStraight(ROAD_LENGTH_SHORT / 2.0);
...
void HelloWorld::render() {
	...
	double playerY = ComUtil::interpolate(playerSegment.getPoint(0).getWorld().getY(),
		playerSegment.getPoint(1).getWorld().getY(), playerPercent);
	double maxy = height;
	Segment segment;
	int n;
	double x = 0;
	...
	if (playerY != 0.0) {
		CCLOG("playerY=%6.6f, Y1=%6.6f, Y2=%6.6f, position=%6.6f", playerY,
			playerSegment.getPoint(0).getWorld().getY(),
			playerSegment.getPoint(1).getWorld().getY(),
			position);
	}
	...
	for (n = 0; n < drawDistance; n++) {
		...
		segment.project((playerX * roadWidth), x, dx, playerY + cameraHeight, 
...
void HelloWorld::player() {
	...
	double updown = playerSegment.getPoint(1).getWorld().getY() - playerSegment.getPoint(0).getWorld().getY();//is climbing hill
	if (steer < 0) {
