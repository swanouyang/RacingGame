...
bool Segment::operator== (Segment segment) {
	if (this->index == segment.getIndex()) {
		return true;
	}
	return false;
}
...
void Segment::setClip(double clip) {
	this->clip = clip;
}

double Segment::getClip() const {
	return this->clip;
}
...
void Segment::project(double cameraX, double x, double dx, double cameraY, double cameraZ, double cameraDepth,
	...
	for (int n = 0; n < MAX_POINTS; n++) {
		...
		p[n].getScreen().setX(round((width / 2.0) + (p[n].getScreen().getScale() * p[n].getCamera().getX() * width / 2.0)));
		p[n].getScreen().setY(round((height / 2.0) - (p[n].getScreen().getScale() * p[n].getCamera().getY() * height / 2.0)));
		p[n].getScreen().setW(round((p[n].getScreen().getScale() * roadWidth * width / 2.0)));
	}
}
...
void Segment::show(double width, double lanes) {
	...
	//if (y1 != y) {
		//CCLOG("y = %6.6f, y1 = %6.6f, y2 = %6.6f\n", y, y1, p[1].getScreen().getY());
	//}
	...
	DRAW_SOLID_RECT(0.0, y2, width, y1 - y2, color.getGrass(), 1.0);
	...
	if (!color.getLane().empty()) {
		...
		lanex1 = x1 - w1 + lanew1;
		lanex2 = x2 - w2 + lanew2;
		for (lane = 1.0; lane < lanes; lanex1 += lanew1, lanex2 += lanew2, lane++) {
			points[0].x = lanex1 - l1 / 2.0;
			points[1].x = lanex1 + l1 / 2.0;
			points[2].x = lanex2 + l2 / 2.0;
			points[3].x = lanex2 - l2 / 2.0;
			DRAW_SOLID_POLY(points, 4, color.getLane());
			//Render.polygon(ctx, lanex1 - l1 / 2, y1, lanex1 + l1 / 2, y1, lanex2 + l2 / 2, y2, lanex2 - l2 / 2, y2, color.lane);
		}
	}

	DRAW_SOLID_RECT(0.0, y2, width, y1 - y2, COLOR_FOG, 1-fog);
	//Render.0og(ctx, 0, y1, width, y2 - y1, fog);
}

double Segment::getRumbleWidth(double projectedRoadWidth, double lanes) {
	return projectedRoadWidth / fmaxf(6.0, 2.0 * lanes);
}

double Segment::getLaneMarkerWidth(double projectedRoadWidth, double lanes) {
	return projectedRoadWidth / fmaxf(32.0, 8.0 * lanes);
}